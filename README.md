...
平台提供四层安全隔离机制（L0-L3），根据不同的执行风险等级采用差异化的隔离策略，确保代码执行的安全性和可靠性。

**沙箱等级定义**：

| 沙箱等级 | 隔离能力 | 适用场景 | 执行限制 | 风险等级 |
|---------|---------|---------|---------|---------|
| **L0 - 主机级访问** | 直接访问宿主机 | 文件操作、系统命令执行 | 白名单目录、敏感文件拒绝、HITL确认 | 高/临界 |
| **L1 - 基础沙箱** | AST静态检查 + 子进程隔离 | 简单脚本执行、数据处理、Skill执行 | 文件系统禁止、网络禁止、白名单标准库 | 低 |
| **L2 - 平台工具** | API层面操作 | 工作流操作、Agent管理、Skill管理 | 受RBAC权限控制、操作审计日志 | 中 |
| **L3 - 高级沙箱** | 专用venv + 工作目录隔离 | 复杂代码执行、数据科学计算、第三方依赖 | 完整网络访问、临时文件系统、资源限制 | 中/高 |

**各层级详细说明**：

**L0 - 主机级访问**：
- 允许直接访问宿主机文件系统和执行shell命令
- 仅允许访问白名单目录（`~/.agentflow` + 当前项目目录）
- 敏感文件自动拒绝（`.env`, `.ssh`, `.pem`, `.key`等）
- 高危操作（写文件、删除、shell命令）需用户确认（HITL机制）
- 包含工具：`host_read`, `host_write`, `host_edit`, `host_delete`, `host_move`, `host_list_dir`, `host_info`, `host_shell`

**L1 - 基础沙箱**：
- AST静态检查：禁止危险模块导入（`os`, `sys`, `subprocess`等）和危险操作（`eval`, `exec`, `open`等）
- 子进程隔离：代码在独立Python进程中执行，超时自动终止（默认5秒）
- 白名单标准库：仅允许24个安全模块（`math`, `re`, `json`, `datetime`等）
- 适用于Skill代码执行和简单数据处理

**L2 - 平台工具**：
- 通过平台API进行操作，受RBAC角色权限约束
- 支持工作流创建/修改/运行、Agent管理、Skill管理、知识库管理
- 角色权限矩阵：super_admin（全部权限）、admin（管理权限）、user（自有资源权限）
- 完整的操作审计日志，支持操作追溯

**L3 - 高级沙箱**：
- 专用venv环境：预装numpy、pandas等数据科学包
- 工作目录隔离：每个会话有独立的工作目录（`~/.agentflow/ws/{session_id}/`）
- Workspace API：内置文件操作API，带路径越界检查
- 完整网络访问：支持HTTP请求（通过`ws.fetch`）
- 资源限制：CPU（60秒）、内存（2GB）、进程数（100-200）、超时（60秒）

**安全特性**：

- **进程隔离**：L1/L3每个执行任务运行在独立进程空间，进程结束后自动清理资源
- **超时保护**：L1默认超时5秒，L3默认超时60秒，超时后自动终止进程
- **资源限制**：CPU、内存、进程数等资源配额控制（L3）
- **路径安全**：L0/L3包含路径越界检查，禁止访问白名单/工作目录以外的文件
- **危险操作拦截**：禁止系统命令执行和敏感文件访问
- **HITL确认**：L0高危操作需用户手动确认
- **执行审计**：完整的执行日志和操作记录

**安全边界**：

| 边界类型 | L0主机级 | L1基础沙箱 | L2平台工具 | L3高级沙箱 |
|---------|---------|-----------|-----------|-----------|
| 文件系统 | 白名单目录 | 禁止访问 | API层面 | 仅限工作目录 |
| 网络访问 | 完整访问 | 禁止 | API层面 | 允许HTTP/HTTPS |
| 系统命令 | 允许（需确认） | 禁止 | 禁止 | 禁止直接执行 |
| 标准库 | 完整访问 | 白名单（24个） | - | 完整访问 |
| 第三方包 | 禁止 | 禁止 | - | 预装包 + 动态安装 |
| 权限控制 | HITL确认 | 静态检查 | RBAC角色 | Workspace API |

**核心API**：

**L0主机工具**：
- `host_read(path)`：读取主机文件（只读，无需确认）
- `host_write(path, content)`：写入主机文件（高危，需确认）
- `host_edit(path, old_str, new_str)`：编辑文件内容（高危，需确认）
- `host_delete(path)`：删除文件/空目录（临界，需确认）
- `host_list_dir(path)`：列出目录内容（只读，无需确认）
- `host_shell(command)`：执行shell命令（视命令类型需确认）
